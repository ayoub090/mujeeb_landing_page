...
import {
  ...
  LockKeyhole, LogOut, MessageCircle, PackageCheck, ShieldCheck, Sparkles,
  TriangleAlert,
} from "lucide-react";
import {api} from "./api";
...
function Dashboard({user,onLogout}:{user:User;onLogout:()=>void}) {
  ...
  const connect=(provider:"salla"|"zid")=>api.post(`/api/integrations/${provider}/start`,{store_id:store.id}).then(r=>location.href=r.data.url);
  ...
  const nav=[{id:"overview",label:"نظرة عامة",icon:BarChart3},{id:"orders",label:"الطلبات",icon:Boxes},{id:"integrations",label:"الربط",icon:Link2},{id:"developer",label:"API المتجر",icon:Code2},{id:"billing",label:"الباقة",icon:CreditCard}];
